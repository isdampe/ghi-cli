...
class GhiUi
{
	...
	async _processComment() {
		try {
			const comment = await this._core.getComment();
			...
			this._setStatusBar("error", "An error occurred posting your comment.");
		}
	...
	_render() {
		...
		switch (this._activeElement) {
			case "list":
				...
				break;
			case "issue":
				stateLine += `{black-fg}{blue-bg} ↑↓ scroll {/blue-bg} {blue-bg} < focus list {/blue-bg} {blue-bg} R add comment {/blue-bg} {blue-bg} C close issue {/blue-bg}{/black-fg}`;
				break;
		}
